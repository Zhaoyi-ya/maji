...
- **截图识别，自动上岛**：对取件码 / 取餐码截图，自动识别并挂到小米 HyperOS **超级岛**（焦点通知）；支持取餐、奶茶、咖啡、取件码、快递柜等类型。
...
- **MiMo 智能识别**：优先使用小米 MiMo 大模型（已有miclaw内测用户可直接登录小米账号使用，账号信息只会保存本地，不会上传云端），未登录mimo账号时回退到标准 OpenAI 兼容 API。
